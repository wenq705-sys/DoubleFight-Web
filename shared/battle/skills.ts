...
export const SKILL_DEFINITIONS: Record<SkillId, SkillDefinition> = {
  random_clear: {
    ...
    shortLabel: '清块',
    icon: '✦',
    description: '随机清除自己两枚未冻结棋子，快速腾出空间。',
  ...
  petrify: {
    ...
    shortLabel: '石化',
    icon: '❄',
    description: '冻结对手一枚真实棋子 6 秒；期间不能移动或合成。',
  ...
  shuffle: {
    ...
    shortLabel: '洗牌',
    icon: '↻',
    description: '重新排列自己的未冻结棋子，保留所有数值。',
...
export function emptySkillCooldowns(): SkillCooldowns {
  return { random_clear: 0, shield: 0, petrify: 0, shuffle: 0, purify: 0 };
}
