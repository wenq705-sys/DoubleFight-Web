...
describe('mobile release performance rescue', () => {
  it('keeps readable DPR independent from visual quality', () => {
    expect(scene).toContain('private currentDpr = 1.75');
    expect(scene).toContain('const dpr = Math.min(this.currentDpr, this.devicePixelRatio)');
  ...
  it('does not construct disabled online rendering or preload every release theme', () => {
