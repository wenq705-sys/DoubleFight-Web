...
describe('shared online game core', () => {
  ...
  it('petrifies a real occupied tile temporarily and restores it after server expiry', () => {
    ...
    const frozenTile = guest.board!.tiles().find((tile) => tile.row === frozen.row && tile.col === frozen.col);
    expect(frozenTile).toBeDefined();
    expect(room.matchSnapshot(now + 1).players.find((player) => player.playerId === guest.id)?.petrifyExpiresAt)
      .toBe(now + 6_000);

    guest.board!.move('left');
    const afterMoveFrozen = guest.board!.tiles().find((tile) => tile.id === frozenTile?.id);
  ...
  it('supports selectable shuffle/purify skills and rejects unequipped skills', () => {
    const room = new RoomSession('565656', () => undefined);
    const host = room.addPlayer('connection-a', 'A', 'kingdom', ['shuffle', 'purify', 'petrify']);
    const guest = room.addPlayer('connection-b', 'B', 'palace', ['purify', 'shield', 'random_clear']);
    room.setReady(host.id, true);
    room.setReady(guest.id, true);
    ...
    room.castSkill(host.id, 'shuffle', 0, 0);
    const after = host.board!.tiles().map((tile) => `${tile.id}@${tile.row},${tile.col}`).join('|');
    expect(after).not.toBe(before);

    guest.board!.blockRandomTile();
    guest.petrifyExpiresAt = 6_000;
    const purified = room.castSkill(guest.id, 'purify', 0, 0);
    expect(purified.event.clearedBlockedCells).toHaveLength(1);
    expect(guest.board!.blockedCells()).toEqual([]);
    expect(guest.petrifyExpiresAt).toBe(0);

    host.energy = 100;
    expect(() => room.castSkill(host.id, 'shield', 1, 1)).toThrow('SKILL_NOT_EQUIPPED');
  });
